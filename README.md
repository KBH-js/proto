...
사내 실프로젝트(웹 기반 데스크톱 MFE 플랫폼)를 아키텍처와 스펙 기반으로 clean-room 재현한 포트폴리오입니다 — 실제 코드는 반출할 수 없으므로, 여기서 보이는 모든 것은 이 레포에서 다시 만들어 실제로 실행됩니다.
...
- **2계층 Remote 구성** — **reference remotes**(Calculator :5001 · Notes :5002)는 최소 MF 계약을 그대로 보여주는 레퍼런스이자 장애 격리 데모의 실험체, **domain remotes**(Network=Neutron :5003 · Compute=Nova :5004)는 TanStack Query 캐싱·장애 주입, MSW-mockable REST(+인메모리 폴백), 스코프 CSS-변수 토큰(호스트 다크 테마 반응), 호스트 locale/theme 브리지를 갖춘 OpenStack 대시보드 — 같은 계약 위에 실제 제품급 화면이 올라간다는 증명
