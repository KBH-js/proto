 ...
 * origin — the app fetches root-relative "/api/neutron/...". db selectors carry
...
export const handlers = [
  http.get('*/api/neutron/networks', () => ok(() => db.listNetworks())),
  http.get('*/api/neutron/routers', () => ok(() => db.listRouters())),
  http.get('*/api/neutron/floatingips', () => ok(() => db.listFloatingIps())),
  http.get('*/api/neutron/networks/:id/ports', ({ params }) =>
    ok(() => db.listPorts(String(params.id))),
  ),
  http.get('*/api/neutron/summary', () => ok(() => db.getSummary())),
];
