...
import { Network as NetworkIcon, RefreshCw, Zap } from 'lucide-react';
...
/**
 * Network dashboard remote (OpenStack Neutron narrative).
 *
...
function Dashboard() {
  ...
  const summaryQ = useQuery({ queryKey: ['summary'], queryFn: networkApi.getSummary });
  const networksQ = useQuery({ queryKey: ['networks'], queryFn: networkApi.listNetworks });
  ...
  return (
    ...
    <div className="remote-network @container flex h-full w-full flex-col overflow-hidden bg-surface font-sans text-body">
      ...
      <div className="grid min-h-0 flex-1 gap-3 p-4 grid-cols-1 grid-rows-[minmax(0,1fr)_minmax(0,1fr)] @2xl:grid-rows-1 @2xl:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)]">
        <section className="flex flex-col overflow-hidden rounded-xl border border-line bg-panel">
          ...
          <div className="flex-1 overflow-auto">
            {networksQ.isPending ? (
              ...
              <ErrorState onRetry={() => networksQ.refetch()} t={t} />
            ) : (
              <NetworkTable
                networks={networks}
        ...
        <section className="flex flex-col overflow-hidden rounded-xl border border-line bg-panel">
          <DetailPanel network={selected} t={t} />
        </section>
      </div>
